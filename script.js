...
var CurrencyInput = function (_React$Component) {
  ...
  _createClass(CurrencyInput, [{
    ...
    value: function render() {
      ...
      //this.props.handleChange = this.handleEuroChange (in the euro case)
      //so when onChange listener fires, it runs the handleChange constant which is equal to this.handleEuroChange, which draws the value from value={value} below, the rate from the CurrencyConverter state, and then the this.toUsd function
...
var CurrencyConverter = function (_React$Component2) {
  ...
  _createClass(CurrencyConverter, [{
    ...
    value: function handleEuroChange(event) {
      var usd = this.convert(event.target.value, this.state.rate, this.toUsd).this.setState({
        euro: event.target.value,
        usd: usd
      });
    }
    // in both of the methods, handleUsdChange and handleEuroChange, we first need to parse the input value into a number. Since the first half of each of the above methods is identical, we can DRY it up!
